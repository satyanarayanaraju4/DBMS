...
select name
from employee
where gender='Female' and department='Operations' and 
salary>(select max(salary) from employee where gender='Male' and department='Testing'
);

---- 


select to_char(date_of_joining,'YYYY') as year,count(*)
from employee 
group by year;


select to_char(date_of_joining,'month') as month,count(*)
from employee 
group by month 
order by month DESC;
...
select name,salary
from employee 
where to_char(date_of_joining,'YYYY')=2021 and salary<(select avg(salary) from employee
where  to_char(date_of_joining,'YYYY')=2020);
...
select department from cte where cnt=(select max(cnt)from cte);
